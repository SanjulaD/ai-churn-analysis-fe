...
export class ApiService {
  static async getChurnOverview(
    ...
    try {
      const [distributionResponse, customersResponse] = await Promise.all([
        ...
        fetch(
          `${API_ENDPOINTS.TOP_RISK_CUSTOMERS}?limit=${limit}&threshold=${threshold}`,
        ),
      ]);
  ...
  static async getRFMSegmentation(): Promise<RFMSegmentationData> {
    try {
      ...
      const validatedDistribution = z
        .array(RFMSegmentDistributionSchema)
        .parse(distribution);
      const validatedPaymentTypes = z
        .array(RFMPaymentTypeSchema)
        .parse(paymentTypes);
      const validatedProducts = z.array(RFMProductSchema).parse(products);
      const validatedReviewScores = z
        .array(RFMReviewScoreSchema)
        .parse(reviewScores);
  ...
  static async getRFMSegmentDetail(
    segmentName: string,
  ): Promise<RFMSegmentDetail> {
    try {
