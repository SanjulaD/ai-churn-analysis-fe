...
import { ChurnDistributionSchema, TopRiskCustomersSchema, ModelMetricsSchema, FeatureImportanceApiSchema, RFMOverviewSchema, RFMSegmentDetailSchema, RFMSegmentDistributionSchema, RFMPaymentTypeSchema, RFMProductSchema, RFMReviewScoreSchema, type RFMSegmentDetail } from '@/schemas/apiSchemas';
import type {
    ChurnData,
...
export class ApiService {
    static async getChurnOverview(limit: number = 10, threshold: number = 0.5): Promise<ChurnData> {
        try {
            ...
            // Validate with Zod
            const validatedDistribution: ChurnDistributionResponse = ChurnDistributionSchema.parse(distributionData);
            const validatedCustomers: TopRiskCustomersResponse = TopRiskCustomersSchema.parse(customersData);

            return {
                churnRate: validatedDistribution.churn_percentage,
                pieData: [
                    { name: 'Will Churn', value: validatedDistribution.churn_percentage, color: '#ef4444' },
                    { name: 'Will Stay', value: validatedDistribution.non_churn_percentage, color: '#10b981' }
                ],
    ...
    static async getModelPerformance(): Promise<ModelPerformanceData> {
        try {
            ...
            return {
                metrics: [
                    { name: 'Accuracy', value: `${(validatedData.metrics.accuracy * 100).toFixed(1)}%`, icon: 'Target', color: 'text-green-600' },
                    { name: 'Precision', value: `${(validatedData.metrics.precision * 100).toFixed(1)}%`, icon: 'Search', color: 'text-blue-600' },
                    { name: 'Recall', value: `${(validatedData.metrics.recall * 100).toFixed(1)}%`, icon: 'Activity', color: 'text-orange-600' },
                    { name: 'F1-Score', value: `${(validatedData.metrics.f1_score * 100).toFixed(1)}%`, icon: 'BarChart3', color: 'text-purple-600' }
                ],
    ...
    static async getRFMSegmentation(): Promise<RFMSegmentationData> {
        try {
            ...
            const [overviewRes, segmentsRes, distributionRes, paymentRes, productsRes, reviewsRes] = await Promise.all([
            ...
            if (!overviewRes.ok || !segmentsRes.ok || !distributionRes.ok || !paymentRes.ok || !productsRes.ok || !reviewsRes.ok) {
                throw new Error('One or more RFM API calls failed');
            }

            const [overview, segments, distribution, paymentTypes, products, reviewScores] = await Promise.all([
                overviewRes.json(),
                segmentsRes.json(),
            ...
            console.log('Raw API responses:', { overview, segments, distribution, paymentTypes, products, reviewScores });
    ...
    static async getRFMSegmentDetail(segmentName: string): Promise<RFMSegmentDetail> {
        try {
            ...
            const response = await fetch(`${API_ENDPOINTS.RFM_SEGMENT_DETAIL}/${encodeURIComponent(segmentName)}`);
